...
* Evaluate the individual and combined effects of Peking- and PI 88788-derived resistance with fluopyram seed treatment on soybean yield and SCN reproduction in Ohio fields infested with the virulent HG Type 1.2.5.7
...
trend = dat %>%
  ...
  scale_fill_manual(
    values = cc, 
    name = expression(atop("Fitted spatial trend", paste("for yield (kg ", ha^{-1}, ")"))),
    drop = FALSE
  ) +
  scale_y_continuous(breaks = c(1, 5, 10, 15, 20, 24)) +
  facet_grid(~year) +
...
eff_data = rbind(eff_st_multi, eff_cult_multi) %>%
  ...
  mutate(
    Comparison = case_when(
      str_detect(Contrast, "Fluopyram vs Base") ~ "Fluopyram vs. base (sub-plot contrast)",
      str_detect(Contrast, "Peking vs PI88788") ~ "Peking vs. PI 88788 (whole-plot contrast)",
      str_detect(Contrast, "Peking \\+ Base vs PI88788 \\+ Fluopyram") ~ "Peking + base vs. PI 88788 + fluopyram (sub-plot contrast)",
      TRUE ~ NA_character_
    )
  )
